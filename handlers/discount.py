...
async def my_discounts(call: CallbackQuery):
    ...
    if coupons:
        ...
        for c in coupons:
            lines.append(
                f"  <code>{c['code']}</code> — "
                f"{c['discount_percent']}% × {c['uses_left']} раз"
            )
    else:
        lines.append("🎟 <b>Купонов нет</b>")

    lines.append("\nℹ️ Купон применяется к следующей покупке.")
...
async def check_coupon_code(message: Message, state: FSMContext):
    ...
    if not coupon or coupon["uses_left"] <= 0:
        await message.answer(
            "❌ Купон не найден или уже использован.",
            reply_markup=back_btn("my_discounts")
        )
        return
