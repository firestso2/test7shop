...
async def process_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"К зачислению на баланс: <b>${amount:.2f}</b>\n"
        f"К оплате (с учётом комиссии): <b>${invoice_amount:.2f}</b>\n\n"
        f"Нажмите «💳 Оплатить» и вернитесь после оплаты.",
        reply_markup=invoice_kb(invoice["bot_invoice_url"]),
        parse_mode="HTML"
    )
...
async def check_payment(call: CallbackQuery, state: FSMContext):
    ...
    if invoice.get("status") == "paid":
        ...
        await update_balance(
            call.from_user.id,
            amount,
            f"CryptoPay invoice#{data['invoice_id']}"
        )
