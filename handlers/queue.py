...
async def refresh_queue(call: CallbackQuery):
    ...
    text = (
        f"📋 <b>Заказ #{order_num}</b> — {order['product_name']}\n\n"
        f"📊 <b>Ваша позиция в очереди:</b>\n"
        f"├ По сервису «{order['product_name']}»: <b>#{pos_item}</b>\n"
        f"└ Общая очередь: <b>#{pos_total}</b>\n\n"
        f"⏳ Администратор выполняет заказы в порядке очереди."
    )

    try:
        await call.message.edit_text(
            text,
            reply_markup=queue_refresh_kb(order_num),
            parse_mode="HTML"
        )
    except Exception:
        pass
