...
async def buy_product(call: CallbackQuery, state: FSMContext):
    ...
    # ── ВЕРИФИКАЦИЯ ЛОГИН:ПАРОЛЬ ──────────────────────────
        ...
        text = (f"✅ <b>Покупка успешна!</b>\n\n"
                ...
                f"После выполнения — введите данные ниже 👇")
        kb = after_purchase_kb(order_num, MANUAL_LINK, False)
        await call.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
        await call.answer("✅ Покупка совершена!")
        # Запускаем флоу сбора логин:пароль прямо в боте
        try:
            from handlers.login_flow import start_creds_flow
            await start_creds_flow(call, item_name, order_num, order_db_id, state)
        except Exception as e:
    ...
    # ── ВЕРИФИКАЦИЯ ПО НОМЕРУ (ЛОГИН-ФЛОУ) ──────────────
        ...
        await call.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
        await call.answer("✅ Покупка совершена!")
        # Запускаем логин-флоу (передаём order_db_id для кнопки «Выполнено»)
        try:
            from handlers.login_flow import start_login_flow
            await start_login_flow(call, item_name, order_num, state)
            # Сохраняем order_db_id и user_id в state для adm:done
            await state.update_data(
                creds_order_db_id=order_db_id,
                creds_user_id=call.from_user.id,
